...
class MY_Model extends CI_Model
{
	...
	public function get_where_row($_table_name,$_primary_key,$_id) 
	{
		$this->db->where($_primary_key,$_id);
		return $this->db->get($_table_name)->row_array();
	}
	public function get_where_row_select($_table_name,$select,$_primary_key,$_id) 
	{
	...
	public function hocphi(){
		return 'Học phí tạm thu kì I năm học';
	}
	public function get_where_select($_table_name,$_primary_key,$_id) 
	{
		$this->db->where($_primary_key,$_id);
		return $this->db->get($_table_name)->result_array();
	}
	public function get_where_select1($_table_name,$select,$_primary_key,$_id) 
	{
		$this->db->select($select);
		$this->db->where($_primary_key,$_id);
		return $this->db->get($_table_name)->result_array();
	...
	public function update_set($table,$column,$key,$set)
	{
		$this->db->where($column,$key);
		$this->db->set($set);
